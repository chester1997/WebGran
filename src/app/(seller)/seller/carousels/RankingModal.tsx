...
export function RankingModal({ rankingCarousel, products }: RankingModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="w-[calc(100%-1.5rem)] sm:w-full sm:max-w-2xl bg-[#121216] border border-white/10 text-white max-h-[90vh] overflow-y-auto overflow-x-hidden custom-scrollbar rounded-2xl shadow-2xl p-4 sm:p-6">
        <DialogHeader className="border-b border-white/5 pb-4 pr-10">
          <DialogTitle className="text-base sm:text-xl font-bold flex items-start gap-2.5 leading-snug">
        ...
        {/* Footer Actions */}
        <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2.5 pt-4 border-t border-white/5">
          <Button
            type="button"
            variant="ghost"
            onClick={() => setOpen(false)}
            className="text-zinc-400 hover:text-white text-xs w-full sm:w-auto"
          >
            Cancelar
          </Button>
          <Button
            onClick={handleSave}
            disabled={isPending}
            className="bg-red-600 hover:bg-red-700 text-white font-semibold text-xs rounded-xl px-5 py-2.5 shadow-lg shadow-red-600/20 w-full sm:w-auto"
          >
            {isPending ? (
