...
export function TopTenCarousel({ storeSlug, products }: TopTenCarouselProps) {
  ...
  return (
    <HorizontalCarousel
      title={
        <h2 className="text-white text-base font-bold tracking-tight">Top 10 Hoje</h2>
      }
    >
      {products.map((product, index) => (
        <Link
          ...
          className="snap-start shrink-0 relative flex items-end group pl-10"
        >
          {/* Outline number positioned to the left, partially behind card */}
          <span
            className="absolute left-0 bottom-0 text-[96px] font-black text-transparent select-none pointer-events-none z-0 leading-none"
            style={{
